 ...
 */
function AdminContent({ initialTab = 'overview' }: { initialTab?: 'overview' | 'reviews' | 'schools' | 'teams' | 'resources' | 'resource-packs' | 'case-studies' | 'users' | 'email-test' | 'evidence-requirements' | 'evidence-triage' | 'events' | 'printable-forms' | 'activity' | 'certificates' }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'reviews' | 'schools' | 'teams' | 'resources' | 'resource-packs' | 'case-studies' | 'users' | 'email-test' | 'evidence-requirements' | 'evidence-triage' | 'events' | 'printable-forms' | 'media-library' | 'data-import' | 'activity' | 'certificates'>(initialTab);
        ...
        <div className="flex overflow-x-auto space-x-1 bg-gray-200 p-1 rounded-lg mb-4 sm:mb-6 lg:mb-8 scrollbar-hide -mx-2 px-2 sm:mx-0">
          ...
          >
            ...
            {((stats && stats.pendingEvidence > 0) || pendingAudits.length > 0 || pendingPhotoConsent.length > 0 || (stats && stats.pendingActionPlansCount > 0)) && (
              <span 
                className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full min-w-5 min-h-5 px-1.5 py-0.5 flex items-center justify-center"
