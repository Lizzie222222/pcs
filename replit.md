...
## Recent Changes
-   **AI-Powered Live Chat Widget**: Implemented floating chat widget available on all pages, appearing as a gradient bubble button (teal to blue) in the bottom-right corner. Features include: (1) OpenAI GPT-4o-mini powered assistant with comprehensive Plastic Clever Schools program knowledge (three-stage process, evidence requirements, resources, events, action plans); (2) Real-time chat interface with message history and auto-scroll; (3) Loading states with spinner animation; (4) Error handling with user-friendly fallback messages; (5) Responsive design with proper focus management and accessibility attributes; (6) Backend endpoint POST `/api/chat` that maintains conversation context and uses detailed system prompt about PCS program. Widget appears globally via App.tsx and uses TanStack Query for API communication. All UI elements include data-testid attributes for testing.
-   **User Profile Management System**: Implemented comprehensive user profile page accessible via dropdown menu from avatar/name in navigation. Features include: (1) Basic profile editing (firstName, lastName, email) with email uniqueness validation; (2) Language preferences that sync with i18n across the entire application; (3) Password change for local auth users (hidden for OAuth-only accounts) with current password verification; (4) GDPR-compliant account deletion with confirmation dialog requiring user to type "DELETE". Backend routes include PUT `/api/user/profile`, PUT `/api/user/language`, PUT `/api/user/password`, and DELETE `/api/user/account`. Navigation component updated with dropdown menu containing Profile, Settings, and Logout options for both desktop and mobile views. Redirect logic uses useEffect to prevent state-update-during-render errors. All mutations invalidate auth cache and provide toast notifications. Profile page route (`/profile`) is authenticated-only and includes comprehensive i18n translations in common.json profile section.
-   **Event Banner Single-Activation Enforcement**: Implemented database-level constraint ensuring only one event banner can be active at any time. Added partial unique index `idx_event_banners_single_active` on `event_banners.is_active` with WHERE clause restricting to active banners only. Storage methods (`createEventBanner`, `updateEventBanner`) wrapped in transactions that deactivate other banners before activating target, using `ne()` operator to exclude current banner. Prevents race conditions and guarantees single active banner even under concurrent activation attempts. Database constraint returns unique violation error if multiple active banners attempted, but transaction logic prevents this scenario by deactivating first.
-   **Contact & Help Center Page Footers**: Added consistent footer components to Contact Us and Help Center pages matching landing/events page design. Fixed navigation overlap issues (Contact: `pt-20` on main container, Help Center: `mt-20` on hero section). Footers include 4-column responsive grid with PCS branding, Program/Support/Connect links, copyright, and Privacy/Terms links, all using i18n translations for multi-language support.
